...
(defn blank_hiccup
  ...
  (let [{:keys [titel beschreibung into-head
    ...
    (list (into [:head
                 ...
                 ;; Site-wide noindex (e.g. staging) is handled by nginx
                 ;; via X-Robots-Tag. Per-page noindex stays here for callers
                 ;; that pass `:noindex? true` (e.g. admin pages).
                 (seo/robots (when noindex? :noindex))
